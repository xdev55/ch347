...
// for setting pin functions
#define CH347_CMD_PIN_SET 0xE2
// for SWD
...
typedef int(__stdcall  * pCH347GetSerialNumber)(unsigned long iIndex, unsigned char *SerialNumberStr);
...
pCH347GetSerialNumber CH347GetSerialNumber;
pCH347GetDeviceInfor CH347GetDeviceInfor;
pCH347GetVersion CH347GetVersion;

/**
 * @brief opens the CH347 device via manufactured driver
 *
...
static int ch347_open_device(void)
{
	...
	if (ch347_device_desc != NULL) {
		for (int i = 0; i < DEVICE_MAX_NUMBER; i++) {
			DevIsOpened = CH347OpenDevice(i);
			if (DevIsOpened != INVALID_HANDLE_VALUE) {
				CH347GetSerialNumber(i, &dev_info);
				if (strcmp(ch347_device_desc, dev_info) == 0) {
					ugIndex = i;
					break;
				} else {
...
/**
 * @brief writes data to the CH347 via manufactured driver
 *
...
static int ch347_write_data(uint8_t *data, int *length)
{
	...
	while (true) {
		transferred = write_len > HW_TDO_BUF_SIZE ? HW_TDO_BUF_SIZE : write_len;
		if (!CH347WriteData(ugIndex, &data[i], &transferred)) {
...
/**
 * @brief reads data to the CH347 via manufactured driver
 *
 * @param data Point to the data buffer
 * @param length Data length in and out
 * @return ERROR_OK at success
...
static int ch347_read_data(uint8_t *data, int *length)
{
	...
	while (true) {
		transferred = read_len > HW_TDO_BUF_SIZE ? HW_TDO_BUF_SIZE : read_len;
		if (!CH347ReadData(ugIndex, &data[i], &transferred)) {
...
{
	return type == CH347_CMD_GPIO || type == CH347_CMD_JTAG_INIT || type == CH347_CMD_SWD_INIT || type == CH347_CMD_PIN_SET;
}
...
static int ch347_srst_enable(void)
{
	int retval = ch347_cmd_start_next(CH347_CMD_PIN_SET);
	if (retval != ERROR_OK)
		return retval;
	uint8_t cmd_data[] = {0x00, 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int ch347_scratchpad_add_pin_byte(void)
{
	return ch347_scratchpad_add_byte(ch347.tms_pin | ch347.tdi_pin | ch347.tck_pin | ch347.trst_pin | ch347.srst_pin);
}
...
static int ch347_init(void)
{
	...
	if (uhModule == 0) {
		...
		if (uhModule) {
			...
			CH347GetSerialNumber = (pCH347GetSerialNumber)GetProcAddress(
				uhModule, "CH347GetSerialNumber");
			CH347GetDeviceInfor = (pCH347GetDeviceInfor)GetProcAddress(
				uhModule, "CH347GetDeviceInfor");
			CH347GetVersion = (pCH347GetVersion)GetProcAddress(
				uhModule, "CH347GetVersion");
			if (CH347OpenDevice == NULL || CH347CloseDevice == NULL
			    || CH347SetTimeout == NULL || CH347ReadData == NULL
			    || CH347WriteData == NULL || CH347GetSerialNumber == NULL
			    || CH347GetDeviceInfor == NULL || CH347GetVersion == NULL) {
